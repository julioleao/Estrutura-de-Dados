...
int main(){
    ...
    printf("Informe a quantidade de registro: ");
    scanf("%d", &quant);
    system(CLEAR);
    client = aloc(client);
    readData(client);
    ...
    do{
        switch(menu()){
            ...
            case 3:
                quit = 0;
                free(client);
                printf("VALEU! FALOU! E ATÉ........ TÉ MAIS.\n\n");
                exit(1);
            default:
                printf("\nOPCAO INVALIDA\n");
...
FichaCliente* realoc(FichaCliente *client){
    ...
    client = (FichaCliente*) realloc (client,((quant + newQuant) * sizeof(FichaCliente)));
